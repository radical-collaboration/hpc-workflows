...
result.file <- paste(rds.folder, '/', str_pad(day.id, 5, pad = 0), '.Rds', sep = '')

if (file.exists(result.file)) {
    stop(paste(result.file, 'exists.'))
} else {
...
print(paste('Reading', analog.files[i]))
nc <- nc_open(analog.files[i])
...
for (i.time.needed in 1:length(times.needed)) {
    if (!file.exists(files.needed[i.time.needed])) {
        print(paste(files.needed[i.time.needed], 'cannot be found. Skip this.'))
        next
    }
...
ret$AnEn$RH <- verifyRankHist(anen, obs, show.progress = T)
